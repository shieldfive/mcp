...
| A local original removed before its vault copy is proven | `vault_move_in` touches an original only after its upload was decrypted back here and matched the bytes read off disk by SHA-256 (a copy that will not decrypt counts as a mismatch), and only if the file is still the same inode, size and mtime; even then it is renamed into `.shieldfive-mcp-trash` with a manifest naming its vault copy, never deleted. The first failure stops the batch and leaves that file and the rest in place. The vault half reaches the disk only through `localSource.mjs`, and writes nothing decrypted | yes, with a corrupt stored copy and a file changed after upload injected |
